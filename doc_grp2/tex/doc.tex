...
% Author: Felix Maaß
	...
	Das Modul wird benachrichtigt, wenn neue Daten an einem Pin anliegen, also zur Verarbeitung zur Verfügung stehen.
	...
	Jeder Filter verfügt über selbst konfigurierbare Filterparameter, die optional während der Laufzeit über die GUI anpassbar sind.
	Diese können zum Beispiel im Code definierte Konstanten ersetzen und die Module damit flexibler machen.
	Eine Art dieses Parameters ist zum Beispiel der boolesche Wert \emph{Debug Mode}, welcher Debug-Ausgaben des jeweiligen Filters in der Konsole aktiviert.
...
% Author: Felix Maaß
	...
	Um einen ADTF-Filter zu implementieren, muss viel Code geschrieben werden, bevor überhaupt mit der wirklichen Logik angefangen werden kann.
...
% Autor: Felix Maaß
	...
	Dazu gab es bereits einen Filter von Audi vorgefertigt, welcher die Umrechnung von Kurvenradius zu Servowert vornahm, allerdings war dieser Code so obskur, dass der Zeitaufwand für das Einlesen in den bestehenden Code den für eine Reimplementierung des Filters nötigen Zeitaufwand aufwiegen würde.
	Auch um das Framework genauer zu verstehen, entschieden wir uns für letzteres Vorgehen.
	
	Für	die Kommunikation mit den Arduinos existiert bereits eine vorgefertigte Filter-Implementation.
...
\label{Steering-Angle-To-Servo}
	...
	Aufgrund des Schlupfes erwiesen sich theoretische Werte für den minimalen Innenradius $x_{min}$ nicht als akkurat, weshalb dieser sodann durch  Praxistests	empirisch ermittelt wurde:
		\[x_{min} = 400mm.\]
...
\chapter{Fazit}
	...
	An vielen Stellen war offensichtlich Code zusammen mit Kommentaren kopiert worden, wobei der Code angepasst wurde, die Kommentare jedoch nicht.
	...
	Hier fiel auf, das bei manchen Abstürzen kein eigener Code verantwortlich war. 
	...
	Bezüglich der Bildverarbeitung fragen wir uns, ob eine Polynomfunktion für die Erkennung der Straßen besser geeignet wäre.
	Diese Zweifel entstanden durch den Sachverhalt, dass die Fahrbahnmarkierungen nicht immer durch Geraden angenähert werden können.
	In einem zukünftigen Projekt stellen wir uns vor, die Repräsentation der Straße auf einer 2D-Karte vorzunehmen und dabei eben diese Polynomfunktionen für eine bessere Darstellung der Kurven zu verwenden.
	Dies ermöglicht auch eine Berechnung des Kurvenradius' unter der Voraussetzung, dass die Kamera ein ausreichend großes Sichtfeld bietet.
	
	Ein weiterer nächster Schritt wäre es, das Bild der Basler-Kamera zu entzerren und dann zusätzlich zur RealSense\texttrademark einzusetzen, um im Nahbereich eine genauere Analyse vornehmen zu können.
	Ist dies gegeben, würden wir die RealSense\texttrademark Kamera wieder in ihre Ausgangsposition zurückversetzen und uns damit einen Überblick über die aktuelle Straßensituation und bevorstehende Gegebenheiten zu verschaffen.
	...
	Hier stellte sich eine große Differenz zwischen Vorwärts- und Rückwärtsfahrt heraus, die durch Spiel in der Aufhängung entsteht.
	
	Die eigentliche Motorregelung ließ sich trotz der schwankenden Werte gut umsetzen, jedoch weiterführende Funktionen wurden schwierig, wie zum Beispiel die Erkennung eines Bremsvorgangs.
	
	\paragraph{Kollisionserkennung.}
	In der Umsetzung der Kollisionserkennung kam es durch die stark variierenden Werte zu erheblichen Problemen. 
	...
	\paragraph{Hinderniserkennung.}
	Hier gibt es ein grundsätzliches Problem bei der Sensorwahl. Da die Ultraschallsensoren alle auf derselben Frequenz senden, kommt es teilweise zu massiven Störungen der Signale und damit der Messwerte.
	Zudem ist einer der Sensoren im Laufe des Praktikum beschädigt worden, sodass er nur noch einen Wert $\sim4cm$ lieferte.
	Um das eigentliche Problem der Reflexionen zu umgehen, wären zusätzliche Infrarotsensoren zur Entfernungsmessung sinnvoll.
